...
SFM_Data = {
    ...
    aerodynamics = {    -- Cx = Cx_0 + Cy^2*B2 +Cy^4*B4                more lift = more drag, B2 and B4 for exponential drag increase with lift
        ...
        Czbe = -0.016,  -- yaw stability vs sideslip                   negative value = resists yaw drift, stabilizes yaw
        cx_gear = 0.07, -- drag from landing gear                      higher value = more drag when gear deployed
        cx_flap = 0.08, -- drag from flaps                             higher value = more drag when flaps deployed
        cy_flap = 0.21, -- lift increase from flaps                    higher value = more lift when flaps deployed
        cx_brk = 0.12,  -- drag from speedbrakes/spoilers              higher value = more drag when speedbrakes/spoilers deployed

        -- M        mach number                                        line from table below is picked according to current aircraft mach speed (interpolated)
        -- Cx0      zero-lift drag coefficient                         higher value = more drag
        -- Cya      lift per degree of AoA                             higher value = more lift at AoA increase, lower value = maintain higher g-loads
        -- B2       lift-induced drag coefficient                      higher value = more drag at higher lift
        -- B4       higher-order lift-induced drag                     higher value = rapid drag rise at higher lift
        -- Omxmax   maximum roll rate                                  higher value = faster roll response
        -- Aldop    maximum AoA before departure                       higher value = more AoA before stall/uncontrolled flight
        -- Cymax    maximum lift coefficient                           caps total available lift
        table_data = {
